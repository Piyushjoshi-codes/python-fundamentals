...
#for loop with range..
n = int(input("Enter the number : ")) #range(starting_num, stop_num, step)
for i in range(n, 10*n + n, n):
    print(i)

#while loop..
key = 5
while key != -1:                     #It is the type of loop that continues till the condition is true
    print("COUNTDOWN : ", key)
    key -= 1    #key = key - 1
print("LAUNCH !!!")
...
for sam in list_sam:
    ...
    if search == sam:
        print("Item found ")
        break                       #breaks the condition after execution
    else: 
